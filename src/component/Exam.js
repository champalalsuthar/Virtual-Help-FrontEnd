...
const Exam = () => {
  ...
  useEffect(()=>{
    AOS.init({ offset: 20,});
    axios.get("https://cuh-virtual-help-backend.vercel.app/getpyq").then((data)=>{
      setpyq(data.data)
    }).catch((err)=>{
      console.error(err);
    })
  ...
  const handelUplaod = async(e)=>{
    ...
    ()=>{
      getDownloadURL(uploadTask.snapshot.ref).then((url)=>{
        ...
        axios.post("https://cuh-virtual-help-backend.vercel.app/pyqs/upload",pyqDetails).then((res)=>{
  ...
  const handelSearch = (e)=>{
    ...
    axios.post("https://cuh-virtual-help-backend.vercel.app/pyqs/search" , Search).then((data)=>{
